...
class Dayreport extends MY_Controller
{
    function __construct()
    {
        ...
        if($this->client){    
            ...
            foreach ($this->view_data['menu'] as $key => $value) { 
                if($value->link == "dayreports"){ $access = TRUE;}
            }
            if(!$this->user->admin) {
            $this->session->set_flashdata('message', 'error:'.$this->lang->line('messages_no_access'));
            redirect('dayreports');
        }
        $this->view_data['submenushiftjournal'] = array(
                         $this->lang->line('application_gt') => 'dayreports/gt',
                         $this->lang->line('application_europark') => 'dayreports/_europark',
                         $this->lang->line('application_sameiet_gt') => 'dayreports/_sameie',
                         $this->lang->line('application_olafiagangen') => 'dayreports/_olafiagangen',
                         );        
...
AND '$year-$thismonth-31' ";
        ...
        $this->content_view = 'dayreport/all';
    }
...
AND '$year-12-31' GROUP BY invoices.paid_date";
        ...
        function gt()
    {
        ...
        $this->view_data['gt'] = $users;
        $this->content_view = 'dayreports/gt';
    }
}