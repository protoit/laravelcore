...
<div id="main">
		...
		<ul class="details span6">
			...
			<li class="<?=$subscription->status;?>"><span><?=$this->lang->line('application_status');?>:</span>
						<a class="label <?php if($subscription->status == 'Active'){ echo 'label-success';}else{echo 'label-important'; } ?>"><?php if($subscription->end_date <= date('Y-m-d') && $subscription->status != "Inactive"){ echo $this->lang->line('application_ended'); }else{ echo $this->lang->line('application_'.$subscription->status);}?></a>			<?php if($subscription->subscribed != "0"){ ?>  <a class="label label-success margin-left-5 tt" title="<?php $unix = human_to_unix($subscription->subscribed.' 00:00'); echo date($core_settings->date_format, $unix);?>" ><?=$this->lang->line('application_subscribed_via_paypal');?></a> <?php } ?>
		...
		<ul class="details span6">
			...
			<li><span><?=$this->lang->line('application_email');?>:</span> <?php if(isset($subscription->company->client->email)){ ?> <?=$subscription->company->client->email;?> <?php }else{echo "-";} ?></li>
		...
		<div class="row">
		<div class="table_head"><h6><i class="icon-list-alt"></i><?=$this->lang->line('application_subscription_items');?></h6></div>
		...
		<?php endforeach;
		if($items == NULL){ echo "<tr><td colspan='5'>".$this->lang->line('application_no_items_yet')."</td></tr>";}
		...
		<tr class="active">
	 	...
	 	<?php if($core_settings->paypal == "1" && $sum != "0.00" && $subscription->subscribed == "0"){ ?><br/>	
			<form action="https://www.paypal.com/cgi-bin/webscr" method="post">
			  ...
			  <input type="hidden" name="return" value="<?=base_url()?>csubscriptions/success/<?=$subscription->id;?>">
	...
	<div class="row">
		...
		<tr id="<?=$value->id;?>" >
			...
			<td><?php if($value->status == "Paid"){echo '<span class="label label-success">'.$this->lang->line('application_'.$value->status);}else{ echo '<span class="label label-warning">'.$this->lang->line('application_open');} ?></span></td>
