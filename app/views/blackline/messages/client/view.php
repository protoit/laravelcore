<?php if($conversation){ 
    ...
    foreach ($conversation as $value):
      ...
      if("c".$this->client->id == $value->sender){ $own = "own";}
      $i = $i+1;
  ?>	
  <div class="message-content <?=$own;?>">
      ...
      "/>
        ...
        <?php if ($id == $value->id && $value->status != "deleted") { ?>
         <a href="<?=base_url()?>cmessages/delete/<?=$value->id;?>" rel="" class="confirm btn btn-mini pull-right ajax-silent btn-danger btn-loader btn-loader-red"><?=$this->lang->line('application_delete');?></a>
         <?php } ?>
 		</div>
