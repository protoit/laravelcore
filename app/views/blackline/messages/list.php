...
<?php if($message){
          ...
          <a href="<?=base_url()?>messages/view/<?=$value->id;?>" id="listmessage<?=$value->id;?>" class="ajax">
...
<div style="height:50px">
      <div class="dataTables_paginate paging_two_button">
           <?php if($message_list_page_prev >= 0){ ?>
            <a href="<?=base_url()?>messages/messagelist/<?=$message_list_page_prev;?><?php if(isset($deleted)){echo $deleted;}?>" class="paginate_enabled_previous message-list"><?=$this->lang->line('application_previous');?></a><?php }else{ ?><a href="#" class="paginate_disabled_previous"><?=$this->lang->line('application_previous');?></a><?php } if($message_list_page_next < $message_rows){ ?><a href="<?=base_url()?>messages/messagelist/<?=$message_list_page_next;?><?php if(isset($deleted)){echo $deleted;}?>" class="paginate_enabled_next message-list"><?=$this->lang->line('application_next');?></a><?php }else{ ?><a href="#" class="paginate_disabled_next"><?=$this->lang->line('application_next');?></a>
          <?php } ?>
        </div>
</div>
